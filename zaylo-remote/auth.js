...
const Auth = {
     ...
     */
    async _doInit() {
        // Check for demo mode in query params first
        ...
        if (sessionStorage.getItem('zaylo-demo-mode') === 'true') {
            console.log('[Auth] 🚀 Running in Demo Mode (Offline Simulation)');
            this.user = { 
            ...
            // Trigger callbacks asynchronously to let page scripts register listeners first
            setTimeout(() => {
                this._onAuthStateChange(this.user);
            }, 100);
            return;
        }
    ...
    async signOut() {
        if (sessionStorage.getItem('zaylo-demo-mode') === 'true') {
            console.log('[Auth] Signing out from Demo Mode');
            sessionStorage.removeItem('zaylo-demo-mode');
            this.user = null;
