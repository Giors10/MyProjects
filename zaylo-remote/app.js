...
const DeviceList = {
  ...
  getAll() {
    const devices = Storage.get(this.STORAGE_KEY, []);
    if (Array.isArray(devices)) {
      ...
      if (hasDuplicates) {
        console.log('[DeviceList] 🧹 Deduplicated stored devices:', deduplicated);
        Storage.set(this.STORAGE_KEY, deduplicated);
      }
      return deduplicated;
    }
    return [];
  ...
  add(device) {
    ...
    const isDemo = typeof sessionStorage !== 'undefined' && sessionStorage.getItem('zaylo-demo-mode') === 'true';
    const id = device.id.toUpperCase().replace(isDemo ? /[^A-Z0-9]/g : /[^A-F0-9]/g, '');
    // Check for duplicate
