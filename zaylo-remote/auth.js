...
// Demo-only Firebase placeholder.
// The portfolio preview runs with window.ZAYLO_PREVIEW_MODE enabled, so Firebase
// is never initialized. Keep this placeholder here so no production config or
// app keys are shipped with the public demo.
const firebaseConfig = {
    apiKey: "DEMO_ONLY",
    authDomain: "demo.invalid",
    projectId: "zaylo-demo",
    storageBucket: "demo.invalid",
    messagingSenderId: "000000000000",
    appId: "demo",
    measurementId: "demo"
};

function isZayloPreviewMode() {
    try {
...
const Auth = {
     ...
     */
    async _doInit() {
        if (isZayloPreviewMode()) {
            this.user = window.ZAYLO_DEMO_USER || {
                uid: 'demo-user',
    ...
    async signOut() {
        ...
        if (isZayloPreviewMode()) {
            this.user = window.ZAYLO_DEMO_USER || this.user;
            return { success: true };
        }
    ...
    _onAuthStateChange(user) {
        if (isZayloPreviewMode()) {
            ...
            if (window.location.pathname.includes('auth.html') || window.location.href.includes('auth.html')) {
                this._redirectToApp();
            }
            return;
        }
