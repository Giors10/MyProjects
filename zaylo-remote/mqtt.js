...
const MQTTClient = {
    ...
    _isPreviewMode() {
        try {
            return window.ZAYLO_PREVIEW_MODE === true ||
                sessionStorage.getItem('zaylo-demo-mode') === 'true';
        } catch (e) {
            return window.ZAYLO_PREVIEW_MODE === true;
        }
    },
    ...
    get connected() {
        if (this._isPreviewMode()) return true;
    ...
    _publish(topic, payload, options = {}) {
        if (this._isPreviewMode()) {
            return this._simulatePreviewPublish(topic, payload);
        }
    ...
    _emitPreviewState(id, state) {
        this.deviceStates.set(id, state);
        if (typeof StateStore !== 'undefined') StateStore.update(id, state);
        this.callbacks.onStateUpdate.forEach(cb => { try { cb(id, state); } catch (e) { } });
    },

    _withPreviewBlindConfidence(state = {}) {
        const calibration = state.calibration && typeof state.calibration === 'object' ? state.calibration : {};
        const positionConfidence = Math.max(96, Number(state.positionConfidence) || 0);
        const calibrationConfidence = Math.max(96, Number(calibration.confidence) || 0);
        return {
            ...state,
            _online: true,
    ...
    _persistPreviewBlindState(id, state) {
        try {
            const key = `blind-state-${id}`;
            ...
            localStorage.setItem(key, JSON.stringify({
                ...
                isOpen: Number(state.position) > 0,
                isCalibrated: true,
    ...
    _simulatePreviewBlindMove(id, targetPosition, baseState = {}) {
        const safeTarget = Math.max(0, Math.min(100, parseInt(targetPosition, 10) || 0));
        const current = this.getDeviceState(id) || {};
        const startRaw = current.position !== undefined ? current.position :
            (current.blindPosition !== undefined ? current.blindPosition : safeTarget);
        let start = Number.parseFloat(startRaw);
        if (!Number.isFinite(start)) start = safeTarget;
        start = Math.max(0, Math.min(100, start));

        if (this._previewMoveTimers.has(id)) {
            clearInterval(this._previewMoveTimers.get(id));
            this._previewMoveTimers.delete(id);
        }

        const {
            position,
            blindPosition,
            targetPosition: ignoredTargetPosition,
            isMoving,
            positionConfidence,
            positionNeedsVerification,
            calibration,
            ...stableBase
        } = baseState || {};
        const inheritedCalibration = calibration && typeof calibration === 'object' ? calibration : current.calibration;
        const commandId = stableBase.commandId || stableBase.lastCommandId || null;
        const commandEcho = commandId ? {
            commandId,
            lastCommandId: commandId,
            lastCommandResult: 'accepted',
            lastCommandTarget: safeTarget
        } : {
            lastCommandResult: 'accepted',
            lastCommandTarget: safeTarget
        };

        const makeFrame = (value, moving) => {
            const live = this.getDeviceState(id) || current || {};
            const liveCalibration = live.calibration && typeof live.calibration === 'object' ? live.calibration : inheritedCalibration;
            const framePosition = Math.max(0, Math.min(100, Math.round(value)));
            return this._withPreviewBlindConfidence({
                ...live,
                ...stableBase,
                ...commandEcho,
                ...
                blindPosition: framePosition,
                isOpen: framePosition > 0,
                isMoving: moving
            });
        };

        const distance = Math.abs(safeTarget - start);
        const initial = makeFrame(start, distance >= 1);
        this._emitPreviewState(id, initial);

        if (distance < 1) {
            const finalState = makeFrame(safeTarget, false);
            this._emitPreviewState(id, finalState);
            this._persistPreviewBlindState(id, finalState);
            return;
        }

        const startedAt = Date.now();
        const duration = Math.max(900, Math.min(2600, 550 + distance * 22));
        const timer = setInterval(() => {
            const elapsed = Date.now() - startedAt;
            const progress = Math.min(1, elapsed / duration);
            const eased = 0.5 - Math.cos(progress * Math.PI) / 2;
            const done = progress >= 1;
            const frameValue = done ? safeTarget : start + ((safeTarget - start) * eased);
            const frame = makeFrame(frameValue, !done);
            this._emitPreviewState(id, frame);
            if (done) {
                clearInterval(timer);
                this._previewMoveTimers.delete(id);
                this._persistPreviewBlindState(id, frame);
            }
        }, 70);
        this._previewMoveTimers.set(id, timer);
    },

    _simulatePreviewPublish(topic, payload = {}) {
        const match = String(topic || '').match(/lumibot\/([A-Fa-f0-9]+)\/(.+)/);
        if (!match) return true;

        const id = match[1].toUpperCase();
        const route = match[2];
        const existing = this.getDeviceState(id) || { _online: true };
        const next = { ...existing, _online: true };

        if (route === 'config/set') {
            const cfg = payload.config && typeof payload.config === 'object' ? payload.config : payload;
            next.config = { ...(existing.config || {}), ...cfg };
        } else if (payload.command === 'getState' || payload.command === 'diagnostics' || payload.command === 'wifiScan') {
            // Keep seeded telemetry intact.
        } else if (payload.command === 'calibrate' && payload.angle !== undefined) {
            next.previewAngle = payload.angle;
        } else {
            Object.assign(next, payload);
        }

        if (payload.light !== undefined) {
            next.light = !!payload.light;
            next.state = next.light ? 'ON' : 'OFF';
        }

        const pos = payload.blindPosition ?? payload.position ?? payload.targetPosition;
        if (pos !== undefined) {
            const safePos = Math.max(0, Math.min(100, parseInt(pos, 10) || 0));
            this.callbacks.onMessage.forEach(cb => { try { cb(topic, JSON.stringify(payload)); } catch (e) { } });
            this._simulatePreviewBlindMove(id, safePos, next);
            return true;
        }

        if (payload.sleep !== undefined) {
            next.isSleeping = !!payload.sleep;
            next.mode = next.isSleeping ? 3 : 0;
            next.sleepStart = payload.sleepStart || (next.isSleeping ? Date.now() : null);
        }

        this._emitPreviewState(id, next);
        this.callbacks.onMessage.forEach(cb => { try { cb(topic, JSON.stringify(payload)); } catch (e) { } });
        return true;
    },
    ...
    getDeviceState(id) {
        const cleanId = id ? id.toUpperCase() : '';
        if (!cleanId) return null;
        return this.deviceStates.get(cleanId) ||
            (typeof StateStore !== 'undefined' && StateStore.get ? StateStore.get(cleanId) : null) ||
            null;
    },
