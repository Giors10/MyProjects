...
function showSunsetConfigModal() {
            ...
            <div class="sunset-location-card" id="sunsetLocationCard">
                ...
                <div class="sunset-location-body" id="sunsetLocBody" style="display:none;">
                    ...
                    <div style="display:flex; gap:8px;">
                        <button id="sunsetLocGpsBtn" type="button" style="flex:1; padding:10px; border-radius:10px; border:1px solid var(--border-glass); background:var(--bg-glass); color:var(--text-secondary); font-size:12px; font-weight:600; cursor:pointer; transition:all 0.2s ease; display:${window.ZAYLO_PREVIEW_MODE ? 'none' : 'flex'}; align-items:center; justify-content:center; gap:6px; font-family:var(--font-family);">
    ...
    function _fetchSunsetForCoords(lat, lon) {
        ...
        if (!OPENWEATHER_KEY) {
            ...
            fallback.setHours(19, 52, 0, 0);
            _sunsetUnix = Math.floor(fallback.getTime() / 1000);
            if (baseTimeEl) baseTimeEl.textContent = _fmtTime(fallback);
    ...
    if (locSetBtn && locInput) {
        locSetBtn.addEventListener('click', () => {
            ...
            if (!OPENWEATHER_KEY) {
                _applyLocationToDevice(51.5072, -0.1276, query || 'London');
                locSetBtn.disabled = false;
                locSetBtn.textContent = 'Set';
                if (typeof Toast !== 'undefined') Toast.info('Demo preview uses a sample location.');
                return;
            }
    ...
    if (gpsBtn) {
        gpsBtn.addEventListener('click', () => {
            if (window.ZAYLO_PREVIEW_MODE) {
                if (typeof Toast !== 'undefined') Toast.info('Demo preview uses a fixed sample location.');
                return;
            }
