...
(function () {
  ...
  function updateHud() {
    const sections = $$("[data-screen-label]");
    if (sections.length) {
      const y = window.scrollY + window.innerHeight * 0.35;
      let cur = sections[0];
      sections.forEach(s => {
      ...
      if (el) el.textContent = (cur.getAttribute("data-screen-label") || "—").toUpperCase();
    }
  ...
  document.addEventListener("click", (e) => {
    ...
    if (segT) setTheme(segT.dataset.val);
  });
  ...
  function bindIdleTimer() {
    ...
    const reset = () => {
      clearTimeout(t);
      t = setTimeout(() => {
        showToast("Still there? Keep scrolling — there's a robot below. 🤖", "IDLE");
      }, 45000);
    };
