...
const DeviceService = {
    ...
    _previewDevices() {
        return (typeof DeviceList !== 'undefined' && DeviceList.getAll)
            ? DeviceList.getAll()
            : [];
    },
    ...
    async subscribeToDevice(homeId, deviceId, onUpdate) {
        ...
        if (this._isPreviewMode()) {
            ...
            setTimeout(() => {
                const dev = this._previewDevices().find(d => d.id === targetCleanId) || null;
    ...
    async subscribeToDevices(homeId, onUpdate) {
        ...
        if (this._isPreviewMode()) {
            setTimeout(() => onUpdate(this._previewDevices().map(d => ({ ...d }))), 0);
            return () => {};
        }
    ...
    async getDevices(homeId) {
        if (!this._initialized) await this.init();
        if (this._isPreviewMode()) return this._previewDevices().map(d => ({ ...d }));
        if (!homeId) {
    ...
    async getDevice(homeId, deviceId) {
        if (!this._initialized) await this.init();
        if (this._isPreviewMode()) {
            const targetCleanId = deviceId ? deviceId.toUpperCase().replace(/[^A-F0-9]/g, '') : '';
            const dev = this._previewDevices().find(d => d.id === targetCleanId);
            return dev ? { ...dev } : null;
        }
        if (!homeId || !deviceId) {
    ...
    async addDevice(homeId, device) {
        if (!this._initialized) await this.init();
        if (this._isPreviewMode()) {
            return typeof DeviceList !== 'undefined' && DeviceList.add ? DeviceList.add(device) : false;
        }
    ...
    async removeDevice(homeId, deviceId) {
        if (!this._initialized) await this.init();
        if (this._isPreviewMode()) {
            return typeof DeviceList !== 'undefined' && DeviceList.remove ? DeviceList.remove(deviceId) : false;
        }
        if (!homeId || !deviceId) {
    ...
    async updateDevice(homeId, deviceId, updates) {
        if (!this._initialized) await this.init();
        if (this._isPreviewMode()) {
            return typeof DeviceList !== 'undefined' && DeviceList.update ? DeviceList.update(deviceId, updates) : false;
        }
    ...
    async saveDeviceOrder(userId, orderedIds) {
        if (!this._initialized) await this.init();
        if (this._isPreviewMode()) {
            try { localStorage.setItem('zaylo-deviceOrder', JSON.stringify(orderedIds)); } catch (e) {}
            return true;
        }
    ...
    async getDeviceOrder(userId) {
        if (!this._initialized) await this.init();
        if (this._isPreviewMode()) {
            try { return JSON.parse(localStorage.getItem('zaylo-deviceOrder') || 'null'); } catch (e) { return null; }
        }
