...
(function () {
  ...
  function initNavSmoothScroll() {
    ...
    document.addEventListener("click", (e) => {
      ...
      const offsetPosition = targetRect - bodyRect - navHeight;
  ...
  // ---- reveal observer ----
  // Trigger reveals a touch BEFORE the element enters the viewport (rootMargin
  // negative-bottom). Combined with the longer CSS curves this makes the
  // animation peak right as the eye lands on it — a more cinematic feel.
  ...
  // Drives:
  //   • each .section::before counter number (subtle vertical drift)
  //   • .hero::after warm glow (slight downward parallax)
  //   • .scroll-hint visibility (fades once the user scrolls)
  // Uses rAF + dirty flag so the work runs at most once per frame
  // and skips entirely when prefers-reduced-motion is set.
  // ============================================================
  function bindParallax() {
    ...
    function update() {
      ...
      sections.forEach(sec => {
        ...
        // Only update sections within ~1.5 viewport heights for perf
        if (r.bottom < -winH * 0.5 || r.top > winH * 1.5) return;
        // Center the section at 0 displacement when its midpoint is mid-viewport
        const sectionMid = r.top + r.height / 2;
        const offset = (sectionMid - winH / 2) / winH; // ~ -1..+1
        // Drift the counter ~40px max in the opposite direction of the section
        const y = Math.max(-60, Math.min(60, -offset * 50));
        sec.style.setProperty("--parallax-y", y.toFixed(1) + "px");
      });

      if (hero) {
        const heroR = hero.getBoundingClientRect();
        // Only animate while hero is in/near view
        if (heroR.bottom > -winH * 0.3) {
          // The further we've scrolled into the hero, the more the glow drifts down.
          const progress = Math.min(1, Math.max(0, scrollY / winH));
          hero.style.setProperty("--hero-parallax", (progress * 70).toFixed(1) + "px");
        }
      }

      if (hint) {
        // Fade the scroll hint once any scroll begins
        if (scrollY > 24) hint.classList.add("dim");
        else hint.classList.remove("dim");
      }
    }
