...
async function init() {
    ...
    // Resolve active home and setup Firebase updates sequentially AFTER Auth is ready
    try {
        if (typeof Auth !== 'undefined') {
            await Auth.waitForAuthReady();
            const user = Auth.getUser();
            if (user) {
                // Ensure activeHomeId is resolved first
                if (typeof HomeService !== 'undefined') {
                    await HomeService.init();
                    const homeId = await HomeService.getActiveHome(user.uid);
                    window.activeHomeId = homeId; // Ensure global activeHomeId is set!
                ...
                // Setup Firebase Metadata Updates (Name changes, etc)
                if (window.DeviceService && typeof DeviceService.subscribeToDevice === 'function') {
                    ...
                    if (homeId) {
                        ...
                        console.warn('[Device] No activeHomeId, skipping Firebase subscription');
                    }
                }
            }
        }
    } catch (e) {
        console.error('[Device] Sequenced initialization failed:', e);
        // Ensure we still connect to MQTT even if Firebase fails in offline mode
        await connectMQTT();
    }
