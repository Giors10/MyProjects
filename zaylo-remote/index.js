...
const GlobalLocation = {
  ...
  canAutoDetect() {
    if (window.ZAYLO_PREVIEW_MODE) return false;
...
function renderSkeletonLoader(list) {
  ...
  for (let i = 0; i < 2; i++) {
    ...
    skeleton.className = 'device-card skeleton-card';
...
function renderDeviceList(devices, list, emptyState, countEl) {
  ...
  list.classList.toggle('is-empty', devices.length === 0);
  if (emptyState) emptyState.classList.toggle('hidden', devices.length !== 0);
  ...
  list.querySelectorAll('.skeleton-card').forEach(card => card.remove());

  if (!devices.length) {
    list.querySelector('.add-card')?.remove();
...
function updateMQTTStatus(connected) {
  ...
  if (status) {
    ...
    if (connected) {
      const isPreviewMobile = window.ZAYLO_PREVIEW_MODE && window.matchMedia('(max-width: 520px)').matches;
      setTimeout(() => status.classList.remove('visible'), isPreviewMobile ? 900 : 3000);
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  document.getElementById('settingsBtn')?.addEventListener('click', () => {
    ...
    const applyDemoSunTimes = () => {
        const now = new Date();
        const sunrise = new Date(now);
        sunrise.setHours(6, 12, 0, 0);
        const sunset = new Date(now);
        sunset.setHours(19, 52, 0, 0);
        _sunriseUnix = Math.floor(sunrise.getTime() / 1000);
        _sunsetUnix = Math.floor(sunset.getTime() / 1000);
        const srEl = modal.querySelector('#sunTimeSunrise');
        const ssEl = modal.querySelector('#sunTimeSunset');
        if (srEl) srEl.textContent = _fmtTime(sunrise);
        if (ssEl) ssEl.textContent = _fmtTime(sunset);
        _updateEffectiveTimes();
        if (sunTimesLoading) sunTimesLoading.style.display = 'none';
    };

    if (storedLat && storedLon && OPENWEATHER_API_KEY) {
    ...
    if (saveGlobalLocationBtn && globalLocationInput) {
        saveGlobalLocationBtn.addEventListener('click', () => {
            ...
            if (!API_KEY) {
                applyGlobalLocation(51.5072, -0.1276, query || 'London');
                saveGlobalLocationBtn.disabled = false;
                saveGlobalLocationBtn.innerHTML = 'Set';
                Toast.info('Demo preview uses a sample location.');
                return;
            }
    ...
    if (useDeviceLocationBtn) {
        if (window.ZAYLO_PREVIEW_MODE) {
            useDeviceLocationBtn.style.display = 'none';
        }
        useDeviceLocationBtn.addEventListener('click', (e) => {
            ...
            if (window.ZAYLO_PREVIEW_MODE) {
                Toast.info('Demo preview uses a fixed sample home location.');
                return;
            }
